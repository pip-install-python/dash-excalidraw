...
dash.register_page(
    ...
    path="/ai-agent",
    name="AI agent",
    description="Prompt → Excalidraw scene via Claude or Gemini",
    order=13,
)
...
layout = dmc.Stack(
    ...
    children=[
        page_header(
            "AI agent",
            "Turn a natural-language prompt into an Excalidraw scene. "
            "Pick a provider, write a prompt, hit generate — the result is "
            "dispatched to the canvas via `command: updateScene` (no "
            "component remount). Run the same prompt twice with different "
            "providers to compare output.",
        ),
        _provider_status(),
        dmc.Paper(
            withBorder=True,
