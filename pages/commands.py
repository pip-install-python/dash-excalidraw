...
dash.register_page(
    ...
    description="Imperative actions via the command prop",
    order=7,
)
...
layout = dmc.Stack(
    ...
    children=[
        page_header(
            "Command dispatch",
            "Every imperative API the upstream Excalidraw exposes is callable "
            "from Python as a `command` prop: a dict with `id`, `type`, and "
            "`payload`. The component dispatches once per unique id, then "
            "clears the prop so React re-renders don't re-fire.",
        ),
        code_block(CODE),
        dmc.Paper(
            withBorder=True,
