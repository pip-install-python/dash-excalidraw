...
dash.register_page(
    ...
    path="/initial-data",
    name="initialData",
    description="Seed elements, appState, and library on first render",
    order=2,
)
...
layout = dmc.Stack(
    ...
    children=[
        page_header(
            "initialData",
            "Pre-populate the canvas with elements, appState overrides, and "
            "optionally library items. The prop is consumed once on mount — "
            "after that, drive changes through the command dispatch.",
        ),
        code_block(CODE),
        dmc.Alert(
            color="yellow",
