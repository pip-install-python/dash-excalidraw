...
def test_page_llms_txt_carries_the_page_prose(client):
    body = client.get("/commands/llms.txt").text
    assert "Command dispatch" in body
    assert "command prop dispatched once per id" in body, (
        "page prose did not reach /<page>/llms.txt"
    )
...
def test_source_directives_are_expanded_in_llms_txt(client):
    ...
    # A string that exists only inside docs/commands/commands.py, so this
    # fails if the directive is echoed rather than expanded.
    assert "from dash_excalidraw import DashExcalidraw" in body, (
...
def test_exactly_one_canonical_tag_for_browsers(client):
    ...
    """
    html = re.sub(r"<!--.*?-->", "", client.get("/commands").text, flags=re.S)
...
# REMOVED (not weakened): the template's version of this test probed
# /networks, a page whose PROSE legitimately names the viewer's `dv-banner`
# class, and pinned that such a mention must not be mis-read as viewer chrome
# leaking into the document. This site has no page that documents the viewer,
# so there is nothing here for it to assert. The CHROME regex it guarded is
# still exercised by test_page_documents_are_not_dead_ends and
# test_the_rendered_view_is_noindex. Restore this test if a page is ever added
