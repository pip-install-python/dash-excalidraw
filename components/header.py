...
from lib.constants import PRIMARY_COLOR, SITE_SHORT_NAME
...
def create_link(icon, href, label):
    ...
    NEVER add ``title=`` beside it: DMC 2.8's ActionIcon/Anchor accept
    ``aria-*`` wildcards but REJECT ``title``, and the TypeError is raised
    during app construction — the whole site fails to boot rather than
    rendering a wrong tooltip. Use ``dmc.Tooltip`` for hover text.
    """
    return dmc.Anchor(
        dmc.ActionIcon(
                    ...
                    [
                        ...
                        dmc.Anchor(
                            dmc.Group(
                                [
                                    html.Img(
                                        # Generated by scripts/make_brand_assets.py,
                                        # same source as the favicon set and the
                                        # social card artwork. The 144px variant is
                                    ...
                                    dmc.Text(
                                        # Read from the one brand constant rather
                                        # than typed here: the template shipped
                                        # "Dash Docs" hard-coded, which is how a
                                        # fork ends up advertising its parent.
                                        SITE_SHORT_NAME,
                                        size="lg",
                                        fw=700,
                                        c=PRIMARY_COLOR,
                ...
                dmc.Group(
                    [
                        ...
                        create_link(
                            ...
                            "View the dash-excalidraw source on GitHub",
                        ),
                        dmc.ActionIcon(
                            [
                                DashIconify(
